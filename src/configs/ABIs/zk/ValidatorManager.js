...
export class ValidatorManager extends SmartContract {
  ...
  getValidatorIndex(p) {
    const isValidator1 = this.compareValidators(
      p,
      this.validator1.getAndRequireEquals()
    );
    const isValidator2 = this.compareValidators(
      p,
      this.validator2.getAndRequireEquals()
    );
    const isValidator3 = this.compareValidators(
      p,
      this.validator3.getAndRequireEquals()
    );
    return Provable.if(
      isValidator1,
      Field(1),
      Provable.if(
        isValidator2,
        Field(2),
        Provable.if(isValidator3, Field(3), Field(0))
      )
    );
  }
